...
contract TakeProfitsHook is BaseHook, ERC1155 {
    //StateLibrary is new here and we havent seen that before
    //its used to add helper functions to the poolmanager to read
    //storage values
    //In this case, we use it for accessing "currentTick" values 
    // from the pool manager
    using StateLibrary for IPoolManager;

    //PoolIdLibrary used to convert PoolKeys to ID'S
...
function placeOrder(
    ...
    // Return the tick at wich the order was actually placed
    return tick;
}
...
function cancelOrder(
    ...
    //NOTE: We dont want to zero this out directly because other users may have
    //the same position
...
function redeem(PoolKey calldata key, int24 tickToSellAt,bool zeroForOne,uint256 inputAmountToClaimFor) external{
    // Get lower actually usable tick for their order
    int24 tick = getLowerUsableTick(tickToSellAt, key.tickSpacing);
    uint256 positionId = getPositionId(key, tick, zeroForOne);
    ...
    // outputAmount = (inputAmountToClaimFor * totalClaimableForPosition) /
    //(totalInputAmountForPosition)

    uint256 outputAmount = inputAmountToClaimFor.mulDivDown(
        totalClaimableForPosition,totalInputAmountForPosition
        );
...
function swapAndSettleBalances(PoolKey calldata key,IPoolManager.SwapParams memory params) internal returns (BalanceDelta){
    // Conduct the swap inside the PoolManager
    BalanceDelta delta = poolManager.swap(key,params, "");

    // If we just did a zeroForOne swap
    // We need to send Token0 to PM, and receive Token 1 from PM
    if (params.zeroForOne) {
        // Negative Value => exact input for output, money leaving user wallet
        //Settle with pool manager
        if (delta.amount0() < 0) {
            _settle(key.currency0, uint128(-delta.amount0()));
        }

        // Positive Value => Money coming into user wallet, exact output for input
        // Take from PM
        if (delta.amount1() < 0) {
...
function executOrder(PoolKey calldata key, int24 tick, bool zeroForOne, uint256 inputAmount) internal {
    ...
    BalanceDelta delta = swapAndSettleBalances(
        ...
        // 'outputAmount' worth of tokens now can be claimed/redeemed by position
        // holders
        claimableOutputTokens[positionId] += outputAmount;
}
}